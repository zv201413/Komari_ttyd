...
| **端口 1 (Port)** | `25774` (访问 Komari 面板) |
| **端口 2 (Port)** | `7681` (TTYD 网页终端) |
| **持久化存储 (Volume)** | 挂载路径填 `/app/data`（必填，否则重启面板数据全丢） |

> ⚠️ **端口变更（2026-07 上游同步后）**：面板端口由 `80` 改为 **`25774`**（上游默认监听 `0.0.0.0:25774`）。老部署升级镜像后如果打不开面板，把平台的端口配置从 80 改成 25774 即可；也可用环境变量 `KOMARI_LISTEN`（如 `0.0.0.0:80`）改回。
